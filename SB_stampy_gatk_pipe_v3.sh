...
ref='/home/owens/ref/Gasterosteus_aculeatus.BROADS1.dna_rm.toplevel.pluspitx1.fa'
stampyref='/home/owens/ref/Gasterosteus_aculeatus.BROADS1.dna_rm.toplevel.pluspitx1'
...
while read prefix
do
	echo "Aligning ${prefix}_R1.fastq"
	$bwa/bwa aln -t 8 $ref $trimmeddata/"$prefix"_R1.fastq 1> $trimmeddata/"$prefix"_R1.sai
	echo "Aligning ${prefix}_R2.fastq"
	$bwa/bwa aln -t 8 $ref $trimmeddata/"$prefix"_R2.fastq 1> $trimmeddata/"$prefix"_R2.sai
	echo "Combining alignments for $prefix"
	$bwa/bwa sampe $ref $trimmeddata/"$prefix"_R1.sai $trimmeddata/"$prefix"_R2.sai $trimmeddata/"$prefix"_R1.fastq $trimmeddata/"$prefix"_R2.fastq 2> $log/$prefix.bwasampe.log | \
	samtools view -Sb - > $bam/${prefix}_premerge.bam
	...
	echo "Aligning orphaned ${prefix}_R1.fastq"
	$bwa/bwa aln -t 8 $ref $unpaired/"$prefix"_unR1.fastq 1> $unpaired/"$prefix"_unR1.sai
	$bwa/bwa samse $ref $unpaired/"$prefix"_unR1.sai $unpaired/"$prefix"_unR1.fastq 2> $log/${prefix}_unR1.bwasamse.log | \
	samtools view -Sb - > $bam/${prefix}_unR1.bam
	echo "Stampy realignment on orphaned ${prefix}_R1.fastq"
	...
	echo "Aligning orphaned ${prefix}_R2.fastq"
	$bwa/bwa aln -t 8 $ref $unpaired/"$prefix"_unR2.fastq 1> $unpaired/"$prefix"_unR2.sai
	$bwa/bwa samse $ref $unpaired/"$prefix"_unR2.sai $unpaired/"$prefix"_unR2.fastq 2> $log/${prefix}_unR2.bwasamse.log | \
	samtools view -Sb - > $bam/${prefix}_unR2.bam
	echo "Stampy realignment on orphaned ${prefix}_R2.fastq"
	...
	echo "Picardtools bam processing of $prefix"
	java -jar $picardtools/MergeSamFiles.jar INPUT=$bam/${prefix}_premerge.stampy.bam INPUT=$bam/${prefix}_unR1.stampy.bam INPUT=$bam/${prefix}_unR2.stampy.bam OUTPUT=/dev/stdout SORT_ORDER=coordinate USE_THREADING=true 2> $log/$prefix.mergesam.log | \
	java -jar $picardtools/CleanSam.jar INPUT=/dev/stdin OUTPUT=/dev/stdout 2> $log/$prefix.cleansam.log | \
	java -jar $picardtools/AddOrReplaceReadGroups.jar I=/dev/stdin O= $bam/$prefix.sortrg.bam SORT_ORDER=coordinate RGID=$prefix RGLB=$project RGPL=ILLUMINA RGPU=$project RGSM=$prefix CREATE_INDEX=True 2> $log/$prefix.addRG.log
